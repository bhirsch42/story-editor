...
function generateWheel({sections, width}) {
  ...
  let sectionSvgs = generateSectionSvgs(sectionAngles);
  let sceneSvgs = generateSceneSvgs(sections, sectionAngles, width);

  sectionSvgs.forEach((sectionSvg, i) => sectionSvg.scenes = sceneSvgs[i]);
